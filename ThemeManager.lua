...
do
    ...
    function ThemeManager:SaveCustomTheme(file)
        if file:gsub(" ", "") == "" then
            Notify("Invalid file name for theme (empty)", 3)
            return
        end
    ...
    function ThemeManager:CreateThemeManager(groupbox)
        ...
        groupbox:AddButton("Create theme", function()
            ...
            if name:gsub(" ", "") == "" then
                Notify("Invalid theme name (empty)", 2)
                return
            end

            self:SaveCustomTheme(name)

            Notify(string.format("Created theme %q", name))
        ...
        groupbox:AddButton("Load theme", function()
            ...
            Notify(string.format("Loaded theme %q", name))
        end)
        ...
        groupbox:AddButton("Delete theme", function()
            ...
            if not success then
                Notify("Failed to delete theme: " .. err)
                return
            end

            Notify(string.format("Deleted theme %q", name))
            self.Library.Options.ThemeManager_CustomThemeList:SetValues(self:ReloadCustomThemes())
            self.Library.Options.ThemeManager_CustomThemeList:SetValue(nil)
        end)
        ...
        groupbox:AddButton("Reset default", function()
            ...
            if not success then
                Notify("Failed to reset default: delete file error")
                return
            end

            Notify("Set default theme to nothing")
            self.Library.Options.ThemeManager_CustomThemeList:SetValues(self:ReloadCustomThemes())
            self.Library.Options.ThemeManager_CustomThemeList:SetValue(nil)
        end)
